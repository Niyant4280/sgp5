    ...
    const busPerformance = await Booking.aggregate([
      ...
      },
      {
        $addFields: {
          occupancyRate: {
            $multiply: [
              { $divide: ['$passengers', { $multiply: ['$capacity', '$bookings'] }] },
              100
            ]
          }
        }
...
router.get('/revenue', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { period = 'month', year = new Date().getFullYear(), operatorId } = req.query;
...
router.get('/operator/:operatorId?', authenticateToken, requireOperator, async (req, res) => {
  try {
    ...
    const operatorBuses = await Bus.find({ operator: operatorId }).select('_id busNumber busType');
    ...
    switch (type) {
      ...
      case 'users':
        data = await User.find({ ...dateFilter, role: 'user' })
          .select('name email phone status createdAt loyaltyPoints');
        filename = 'user_data';
        break;
