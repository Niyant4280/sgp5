...
router.post('/broadcast', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { title, message, type = 'announcement', priority = 'medium', targetRole } = req.body;
    ...
    // Get target users
    const query = targetRole ? { role: targetRole, status: 'active' } : { status: 'active' };
...
router.post('/send', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { userId, title, message, type = 'custom', priority = 'medium', data } = req.body;
...
async function sendBoardingReminders() {
  try {
    ...
    upcomingBookings.forEach(booking => {
      ...
      if (timeDiff < 5 * 60 * 1000) { // Within 5 minutes of 30-minute mark
