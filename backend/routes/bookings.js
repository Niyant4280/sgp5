  ...
  try {
    ...
    if (!busId || !routeId || !passengers || !journey || !contact) {
      return res.status(400).json({
        error: {
          message: 'Bus, route, passengers, journey details, and contact are required',
    ...
    const unavailableSeats = requestedSeats.filter(seat => bookedSeats.has(seat));
    ...
    const invalidSeats = requestedSeats.filter(seat => !validSeats.includes(seat));
    ...
    const booking = new Booking({
      ...
      passengers: passengers.map(passenger => ({
        ...passenger,
        ticketType: passenger.age < 12 ? 'child' : passenger.age > 60 ? 'senior' : 'adult'
      })),
      journey: {
        ...journey,
        date: journeyDate,
        estimatedArrivalTime: route.getNextDeparture(new Date(journeyDate.getTime() + (route.duration.estimated * 60 * 1000)))
      },
      pricing: {
...
router.get('/:bookingId', authenticateToken, async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.bookingId)
      ...
      .populate('route', 'name origin destination duration distance intermediateStops');
...
// Check-in passenger
router.put('/:bookingId/checkin', authenticateToken, requireOperator, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && booking.bus.operator.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        error: {
  ...
  try {
    ...
    res.json({
      statistics: {
        ...
        averageSpentPerBooking: userStats.totalBookings > 0 
