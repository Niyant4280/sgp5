...
routeSchema.virtual('costPerKm').get(function() {
  if (this.distance.total > 0) {
    return Math.round((this.pricing.basePrice / this.distance.total) * 100) / 100;
  }
  return 0;
});
...
routeSchema.methods.addRating = function(rating) {
  ...
  const totalRating = this.popularity.rating.average * this.popularity.rating.count + rating;
